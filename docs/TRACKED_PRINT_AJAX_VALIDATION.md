...
The native capture-phase form listener calls the same form-scoped validator as defense in depth. Enter in an eligible tracked-print text or URL input calls the form's native `requestSubmit()`; textareas retain normal multiline behavior. Normal Add to cart, sticky Add to cart, Enter-key submission, and `requestSubmit()` therefore share one validation path. Forms without rendered tracked-print controls pass through unchanged, including unrelated product forms and ordinary quick add.
